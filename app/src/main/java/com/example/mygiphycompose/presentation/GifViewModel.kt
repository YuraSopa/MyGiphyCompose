...
class GifViewModel @Inject constructor(
    ...
    private fun getCachedGifFile(context: Context, url: String, callback: (File?) -> Unit) {
        Glide.with(context)
            .asFile()
            .load(url)
            ...
            .into(object : CustomTarget<File>() {
                override fun onResourceReady(
                    ...
                    val gifFile = File(resource.parent, "${resource.name}.gif")
                    resource.renameTo(gifFile)
                    callback(gifFile)
                }
    ...
    fun shareGif(context: Context, url: String) {
        getCachedGifFile(context, url) { file ->
            if (file != null) {
                val uri = FileProvider.getUriForFile(
                    context, "${context.packageName}.fileprovider", file
                )

                val shareIntentMult = Intent(Intent.ACTION_SEND_MULTIPLE).apply {
                    type = "image/gif"
                    putParcelableArrayListExtra(Intent.EXTRA_STREAM, arrayListOf(uri))
                    addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
                }
                context.startActivity(Intent.createChooser(shareIntentMult, "Share GIF"))
            }
        }
    }
